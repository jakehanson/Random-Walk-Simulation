...
{
	/* Define Variables */
	double L = 3.;  // box length
	double H = 3.;  // box height
	...
	params << "L\tH\ta\ttemp\tr_enc\tnum_ants\tcollision_flag" << std::endl;
	params << L << "\t" << H << "\t" << a << "\t" << temp << "\t" << r_enc << "\t" << num_ants << "\t"
	...
	try{
	ants.populate(L,H,temp,r_enc,max_init); // initialize positions and velocities
	}
	...
	while(all_clear == false){
		...
		t_wall = get_t_wall(ants,L,H,r_enc,index1); // how long until colliding with wall?
		if(collision_flag){
			t_ant = get_t_ant(ants,r_enc,t_wall,index2,index3); // how long before colliding with another ant?
		}
		if(collision_flag && t_ant < t_wall){
			ants.update(t_ant,r_enc,index2,index3);
		}
		else{
			ants.update(L,H,a,t_wall,index1);
		}
