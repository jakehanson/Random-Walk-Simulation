...
long double get_t_wall(Ants ants,long double R,long double r_enc,int &index1,long double t_entry,long double machine_tol){
	...
	long double distance; // used to make sure particle is in the box
	int counter = 0;
	int max_tries = 10;
	bool all_clear = false; // used to correct for numerical imprecision
	long double x_new,y_new;

	t_min = 2.*t_entry; // set minimum time before event to be the next entry time
	...
	for(int i=0; i<num_ants; ++i){
		if(ants.nest_flag[i]){
			...
			// If we are inside the arena, the time calculation will be normal
			if(distance <= R - r_enc){
				t_collide = (-1*(x*v_x+y*v_y)+
						sqrt(pow(x*v_x+y*v_y,2)-(pow(v_x,2)+pow(v_y,2))*(pow(x,2)+pow(y,2)-pow(R-r_enc,2))))
						/(pow(v_x,2)+pow(v_y,2));
			}
			// If we are outside due to numerical imprecision, we have to step back in before calculating wall time
			else{
				all_clear = false;
				while(counter < max_tries){
					// baby step back into arena
					x_new = x+v_x*machine_tol;
					y_new = y+v_y*machine_tol;
					distance = std::sqrt(pow(x_new,2)+pow(y_new,2));
					if(distance <= R-r_enc){
						// we are back in and t_collide is legit
						t_collide = (-1*(x*v_x+y*v_y)+
							sqrt(pow(x*v_x+y*v_y,2)-(pow(v_x,2)+pow(v_y,2))*(pow(x,2)+pow(y,2)-pow(R-r_enc,2))))
						/(pow(v_x,2)+pow(v_y,2));
						all_clear = true;
	...
	for(int i=0;i<num_ants;++i){
		if(ants.nest_flag[i]){
			for(int j=i+1;j<num_ants;++j){
				if(ants.nest_flag[j]){
					// Make sure its not the same ants as before
					if(i != index2 or j != index3){
						...
						if(v_dot_r < 0. and d > 0. and r_dot_r > pow(sigma,2)){
							...
							// Check if collision is next eent
							if(t_collide < t_min){
								t_min = t_collide;
...
void Ants::update(long double t_entry,long double R, long double r_enc){
	...
	for(int i=0;i<x_positions.size();++i){
		if(nest_flag.at(i)){
			...
			double separation;
			separation = std::sqrt(pow(x_positions.at(i),2)+pow(y_positions.at(i)-(R-r_enc),2));
			if(separation < 2*r_enc){
				std::cout << x_positions.at(i) << "\t" << y_positions.at(i) << "\t" << separation << std::endl;
...
void Ants::update(long double R,long double r_enc, long double a,long double t_min,int index1,long double machine_tol){
	...
	for(int i=0;i<num_ants;++i){
		...
		if(i == index1){
			...
			long double distance;
			distance = std::sqrt(pow(x_new,2)+pow(y_new,2));
			if(std::abs(distance - 1.9) > machine_tol){
				std::cout << "\t Wall Distance = " << std::setprecision(20) << distance << std::endl;
				throw std::runtime_error("WALL COLLISION NOT PUTTING PARTICLE ON WALL!");
			}
